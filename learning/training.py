...
def validate(encoder, decoder, val_loader: torch.utils.data.DataLoader, tokenizer: GPT2Tokenizer, logger, steps, training_arguments, sample_size: int = 5, cala_p_t_sore=False, prefix: str = "", eval_opts: List[str]=None):
    """
    ...
    decoder.eval()
    with torch.no_grad():
        evaluate(encoder, decoder, val_loader, tokenizer, steps, logging_wandb=~training_arguments.get_turnoff_wandb(), sample_size=sample_size, cala_p_t_sore=cala_p_t_sore, prefix=prefix, eval_opts=eval_opts)
...
def validate_and_save_model(encoder, decoder, optimizer, full_val_loader, cleaned_val_loader, tokenizer, steps: int, training_arguments, logger = None, cala_p_t_sore=True):
    ...
    if cleaned_val_loader is not None:
        ...
        # To generate this dataset, we hand-picked 251 human generated instructions from the full validation set, in which the shortest pathes generated by our system planner aligns well with the instructions.
        validate(encoder, decoder, cleaned_val_loader, tokenizer, logger, steps,training_arguments, sample_size=10, cala_p_t_sore=cala_p_t_sore, prefix="Cleaned-dev", eval_opts=eval_opts)

    if full_val_loader is not None:
        # validate on full human-human interaction validation data
        validate(encoder, decoder, full_val_loader, tokenizer, logger, steps, training_arguments, sample_size=3, cala_p_t_sore=cala_p_t_sore, prefix="Full-dev", eval_opts=eval_opts)
...
def train(training_arguments: TrainingArgs):
    ...
    if (not training_arguments.get_validate_only()) and train_configs["skip_validation_at_step_zero"]:
    ...
    # training loop
    if not training_arguments.get_validate_only():
